---
name: todo-skill
description: Create, split, normalize, and resume recursive self-contained TODO task trees with materialized effective rules per node. Use when an agent needs to create a root task, add a child task/subtask, sync inherited rules, or leave a task resumable without relying on prior chat memory. Prefer storing project-local task assets, project rules, and skill config under the project's TODO folder.
---
